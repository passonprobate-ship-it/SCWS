...
async function pm2ProcessExists(pm2Name: string): Promise<boolean> {
  try {
    ...
    return JSON.parse(stdout).some((p: { name: string }) => p.name === pm2Name);
  } catch {
    return false;
  }
}
...
export async function pm2Start(
  ...
  // Delete stale process before re-start
  if (await pm2ProcessExists(pm2Name)) {
    await execFileAsync("pm2", ["delete", pm2Name], { timeout: 10_000 });
    log(`PM2 deleted stale process before re-start: ${pm2Name}`, "pm2");
  }

  const memRestart = Math.round(heapMB * 1.2);

  if (startCommand) {
    await execFileAsync("bash", ["-c",
      `cd "${cwd}" && env ${envArgs} pm2 start bash --name "${pm2Name}" --cwd "${cwd}" --update-env --max-memory-restart ${memRestart}M -- -c "cd '${cwd}' && ${startCommand}"`,
    ], { timeout: 30_000 });
  } else {
    await execFileAsync("bash", ["-c",
      `cd "${cwd}" && env ${envArgs} pm2 start "${entryFile}" --name "${pm2Name}" --cwd "${cwd}" --update-env --node-args="--max-old-space-size=${heapMB}" --max-memory-restart ${memRestart}M`,
    ], { timeout: 30_000 });
  }

  log(`PM2 started: ${pm2Name} on port ${port} (heap ${heapMB}MB)`, "pm2");
}
